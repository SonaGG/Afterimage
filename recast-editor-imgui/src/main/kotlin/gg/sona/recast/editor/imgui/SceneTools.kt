...
import gg.sona.recast.editor.imgui.SceneMath.axisOffset
import gg.sona.recast.editor.imgui.SceneMath.planePoint
import gg.sona.recast.editor.imgui.SceneMath.pointInPolygon
import gg.sona.recast.editor.imgui.SceneMath.segmentDistance
...
class SceneTools(private val context: EditorContext) {
    ...
    private fun contextMenu(session: EditorSession) {
        ...
        try {
            ...
            ImGui.separator()
            if (ImGui.menuItem("Go to keyframe")) session.replay?.seek(time)
            if (ImGui.menuItem("Look through")) lookThrough(frame.pose)
            if (ImGui.menuItem("Frame", "F")) context.host.camera.frame(
            ...
            if (ImGui.beginMenu("Interpolation")) {
                for (mode in SegmentMode.entries) if (ImGui.menuItem(
                        mode.label,
                        "",
                        frame.mode == mode
                    )
