...
class GameLanes(private val context: EditorContext, private val geometry: TimelineGeometry) {
    ...
    fun momentsLaneMenu(session: EditorSession) {
        ...
        if (ImGui.menuItem("Add moment at playhead")) addManual(session, session.playheadNanos)
        if (ImGui.menuItem("Keep all detected moments", "", false, detected.isNotEmpty())) {
            for (moment in detected) keep(session, moment)
            context.toast("Kept ${detected.size} moments")
        }
        if (ImGui.menuItem("Clips from all moments", "", false, all.isNotEmpty())) {
    ...
    fun playersLaneMenu() {
        ...
        if (ImGui.menuItem(if (anyExpanded) "Collapse all" else "Expand all")) for (row in rows) row.expanded =
            !anyExpanded
        if (rows.size > MAX_ROWS && ImGui.menuItem(if (showAllPlayers) "Show the ${MAX_ROWS} most present" else "Show all ${rows.size} players")) showAllPlayers =
            !showAllPlayers
        ImGui.separator()
        Widgets.smallText(
    ...
    fun worldLaneMenu() {
        for (row in WorldRow.entries) {
            ...
            if (ImGui.menuItem(row.label, "", row !in hiddenWorldRows)) {
