...
class CameraPanel(private val context: EditorContext) : AbstractPanel("Camera", DockArea.RIGHT, Icon.CAMERA) {
    ...
    private fun aimPicker(session: EditorSession) {
        ...
        if (Widgets.popupButton("aim", label)) {
            if (ImGui.menuItem("Keyframe rotation", "", current == null)) session.execute(SetAimTarget(null))
            if (ImGui.menuItem(recorderName, "", current == CameraSettings.TARGET_RECORDER)) session.execute(
            ...
            for (entity in players) {
                if (ImGui.menuItem(entityLabel(entity.id), "", current == entity.id)) session.execute(
                    SetAimTarget(
                        entity.id
                    )
                )
    ...
    private fun targetPicker(settings: CameraSettings, control: CameraControl) {
        ...
        if (Widgets.popupButton("target", label)) {
            ...
            if (players.isNotEmpty()) {
                ImGui.separator()
                for (entity in players) {
                    if (ImGui.menuItem(entityLabel(entity.id), "", settings.targetEntityId == entity.id)) {
