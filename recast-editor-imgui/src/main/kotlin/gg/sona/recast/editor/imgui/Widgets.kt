...
object Widgets {
    ...
    fun dottedWidth(parts: List<String>): Float {
        var width = 0f
        for ((index, part) in parts.withIndex()) {
            if (index > 0) width += ImGui.getFontSize() * 0.5f + ImGui.getStyle().itemSpacingX * 2f
            width += textWidth(part)
        }
        return width
    }

    fun dotted(parts: List<String>, color: Int = EditorTheme.TEXT_DIM.u32) {
        if (parts.isEmpty()) {
