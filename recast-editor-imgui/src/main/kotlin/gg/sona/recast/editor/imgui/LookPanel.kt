...
import gg.sona.recast.editor.commands.SetLook
...
class LookPanel(private val context: EditorContext) : AbstractPanel("Look", DockArea.RIGHT, Icon.PALETTE) {
    ...
    private fun focusPicker(session: EditorSession, look: LookSettings) {
        ...
        if (Widgets.popupButton("focus-target", label)) {
            if (ImGui.menuItem("Focus distance", "", current == null)) edit(session, "focusTarget") { it.focusTargetId = null }
            if (ImGui.menuItem(recorderName, "", current == CameraSettings.TARGET_RECORDER)) edit(session, "focusTarget") {
            ...
            for (entity in players) {
                if (ImGui.menuItem(entityLabel(entity.id), "", current == entity.id)) edit(session, "focusTarget") {
    ...
    private fun grade(session: EditorSession, look: LookSettings) {
        ...
        if (Widgets.beginProperties("look-grade")) {
            ...
            Widgets.property("LUT", "Colour lookup table (.cube) from the recast/luts folder")
    ...
    private fun lutPicker(session: EditorSession, look: LookSettings) {
        ...
        if (Widgets.popupButton("lut", label)) {
            val files = scanLuts()
            if (ImGui.menuItem("None", "", look.lut.isEmpty())) edit(session, "lut") { it.lut = "" }
            if (files.isNotEmpty()) ImGui.separator()
            for (file in files) {
                if (ImGui.menuItem(file.removeSuffix(".cube").removeSuffix(".CUBE"), "", file == look.lut)) edit(session, "lut") {
                    it.lut = file
                }
            }
            if (files.isEmpty()) Widgets.mutedText("Drop .cube files into recast/luts")
            ImGui.separator()
            if (ImGui.menuItem("Open LUT folder")) openFolder(context.host.lutsDirectory)
            if (ImGui.menuItem("Rescan")) lutsScannedNanos = 0L
            Widgets.endPopup()
        }
    }
