    ...
    fun menu(context: EditorContext, session: EditorSession, moment: Moment, renameBuffer: ImString) {
        ...
        if (kept) {
            ...
            if (ImGui.beginMenu("Kind")) {
                for (kind in MomentKind.entries) if (ImGui.menuItem(
                        kind.label,
                        "",
                        moment.kind == kind
                    )
        ...
        if (ImGui.menuItem("Play moment")) play(session, moment)
        if (ImGui.menuItem("Go to peak")) replay.seek(moment.peakNanos)
        if (ImGui.menuItem("Set in/out to moment")) session.execute(SetInOutPoints(moment.nanos, moment.endNanos))
        ImGui.separator()
        if (!kept && ImGui.menuItem("Keep moment")) keep(session, moment)
        if (ImGui.menuItem("Clip from moment")) clipFrom(session, moment)
        if (ImGui.menuItem("Add marker at peak")) session.execute(
            AddMarker(
                TimelineMarker(
                    UUID.randomUUID(),
                    moment.peakNanos,
        ...
        if (moment.entityId >= 0 && ImGui.menuItem("Search this player")) {
        ...
        if (kept) {
            ImGui.separator()
            if (ImGui.menuItem("Remove moment", "Del")) {
