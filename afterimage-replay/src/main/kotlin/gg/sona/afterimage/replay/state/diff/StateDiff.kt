package gg.sona.afterimage.replay.state.diff
...
import gg.sona.afterimage.replay.state.shadow.*
...
object StateDiff {
    ...
    private fun diffMaps(from: ShadowClient, to: ShadowClient, out: MutableList<ClientboundPacket>) {
        for (canvas in to.world.maps.values) {
            if (!canvas.hasPixels) continue
            val previous = from.world.maps[canvas.mapId]
