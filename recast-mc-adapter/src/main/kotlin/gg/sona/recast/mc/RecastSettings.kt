...
class RecastSettings(private val file: Path) {
    ...
    fun load() {
        ...
        runCatching { Files.newBufferedReader(file).use { values.load(it) } }
            .onFailure { logger.warn("Recast could not read settings {}", file, it) }
    }
    ...
    fun saveIfDirty() {
        ...
        runCatching {
            ...
            Files.newBufferedWriter(file).use { values.store(it, "Recast settings") }
        }.onFailure { logger.warn("Recast could not write settings {}", file, it) }
    }
