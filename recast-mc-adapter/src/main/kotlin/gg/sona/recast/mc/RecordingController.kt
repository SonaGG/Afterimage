...
import gg.sona.recast.capture.packet.PacketCapture
...
class RecordingController(
    ...
    fun onPlayConnection(connection: Connection) {
        ...
        val deriver = EventDeriver(shadow)
        ...
        capture.listeners.add(object : CaptureListener {
            ...
            override fun onFailure(stage: String, error: Throwable) {
                logger.error("Recast capture stage {} failed", stage, error)
            }
        })
    ...
    fun onClientTickStart() {
        ...
        session.capture.offer(PacketDirection.SERVERBOUND, RecastInternal.LOCAL_TICK, EMPTY_PAYLOAD)
    }
    ...
    fun onDisconnect() {
        ...
        runCatching { session.capture.close() }.onFailure { logger.error("Recast failed to close capture", it) }
    }
    ...
    fun instantClip(): Clip? {
        ...
        runCatching { clips.save(clip) }.onFailure { logger.warn("Recast could not persist instant clip", it) }
    ...
    private fun registerChannel(player: LocalClientPlayerEntity) {
        ...
        runCatching {
            val buffer = PacketByteBuf(Unpooled.wrappedBuffer(RecastChannel.registrationPayload()))
            handler.sendPacket(CustomPayloadC2SPacket(RecastChannel.REGISTER_CHANNEL, buffer))
        }.onFailure { logger.warn("Recast could not register the ${RecastChannel.NAME} channel", it) }
    }
    ...
    fun onFrame(tickDelta: Float) {
        ...
        session.capture.offer(PacketDirection.SERVERBOUND, RecastInternal.LOCAL_POSE, poseWriter.toByteArray())
    }
    ...
    fun onLocalBlockChange(position: Long, state: Int) {
        ...
        session.capture.offer(PacketDirection.SERVERBOUND, RecastInternal.LOCAL_BLOCK_CHANGE, poseWriter.toByteArray())
    }
    ...
    private fun sampleTarget(session: Live) {
        ...
        session.capture.offer(PacketDirection.SERVERBOUND, RecastInternal.LOCAL_TARGET, poseWriter.toByteArray())
    }
    ...
    private fun sampleScreen(session: Live, allowMouse: Boolean) {
        ...
        session.capture.offer(PacketDirection.SERVERBOUND, RecastInternal.LOCAL_SCREEN, poseWriter.toByteArray())
    }
    ...
    fun onBlockMiningProgress(breakerId: Int, position: Long, stage: Int) {
        ...
        session.capture.offer(PacketDirection.SERVERBOUND, RecastInternal.LOCAL_BLOCK_BREAK, poseWriter.toByteArray())
    }
    ...
    fun onFrameEnd() {
        ...
        session.capture.offer(
            ...
            RecastInternal.CAMERA_FRAME_COMPACT,
    ...
    private fun onClipRequest(capture: PacketCapture, request: ClipRequest) {
        ...
        val clip = request.toClip(info.path, info.sessionId, info.originNanos)
        runCatching { clips.save(clip) }.onFailure { logger.warn("Recast could not persist flashback clip", it) }
        logger.info("Recast flashback: {} at {} ms", clip.title, clip.startNanos / 1_000_000L)
        flashbackListeners.dispatch { it.onFlashbackClip(request, clip) }
    }
