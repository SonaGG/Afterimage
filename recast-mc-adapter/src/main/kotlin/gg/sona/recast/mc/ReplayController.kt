...
import gg.sona.recast.protocol.ResourcePackSend
...
import gg.sona.recast.replay.perspective.RecorderProjection
import gg.sona.recast.replay.session.ReplayListener
import gg.sona.recast.replay.session.ReplayOptions
...
class ReplayController(private val platform: MinecraftPlatform, val camera: CameraDriver) {
    ...
    private fun applyServerPack(url: String, hash: String) {
        ...
        logger.info("Recast applying the recorded server resource pack {}", url)
        packReopenFrames = 40
        runCatching {
        ...
        }.onFailure { logger.warn("Recast could not apply the server resource pack", it) }
    }
    ...
    fun open(target: Path): Boolean {
        ...
        return try {
            ...
            replay.play()
            applyTimerScale()
            logger.info("Recast opened replay {} ({} s)", target.fileName, replay.durationNanos / Nanos.PER_SECOND)
            true
        } catch (error: Throwable) {
            lastError = error.message ?: error.javaClass.simpleName
            logger.error("Recast could not open replay {}", target, error)
            close()
            false
        }
    }
    ...
    fun abandon() {
        ...
        logger.warn("Recast replay abandoned because the client joined another world")
    ...
    private fun close(leaveWorld: Boolean) {
        ...
        restoreViewDistance()
        camera.detach()
        runCatching { replay.close() }.onFailure { logger.error("Recast failed to close replay", it) }
        val connection = mirror
        mirror = null
        appliedPack = null
        packReopenFrames = 0
        (minecraft as MinecraftAccessor).`recast$timer`().tpsScale = 1f
        if (leaveWorld) {
            ...
            runCatching { connection?.close() }.onFailure {
                logger.warn(
                    "Recast could not close the replay connection",
                    it
                )
            }
        }
    ...
    fun ownsWorld(): Boolean {
        ...
        val connection = (current as? ReplayConnectionOwner)?.`recast$connection`() ?: return true
        return RecastHooks.isReplayConnection(connection)
    }
    ...
    private fun applyRecordedViewDistance(replay: ReplaySession, now: Long) {
        ...
        var radius = 0
        replay.shadow.world.chunks.forEach { _, chunk ->
    ...
    private fun applyTimerScale() {
        val replay = session ?: return
        (minecraft as MinecraftAccessor).`recast$timer`().tpsScale = if (replay.playing) replay.speed.toFloat() else 1f
    }
