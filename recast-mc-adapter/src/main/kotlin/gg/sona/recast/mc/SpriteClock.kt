...
import gg.sona.recast.mc.mixin.TextureAtlasAccessor
import gg.sona.recast.mc.mixin.TextureAtlasSpriteAccessor
...
class SpriteClock(private val minecraft: Minecraft) {
    ...
    fun align(tickIndex: Long) {
        ...
        val sprites = (atlas as? TextureAtlasAccessor)?.`recast$animatedSprites`() ?: return
        var bound = false
        for (sprite in sprites) {
            val accessor = sprite as? TextureAtlasSpriteAccessor ?: continue
            val animation = accessor.`recast$animation`() ?: continue
            val frames = accessor.`recast$frames`()
            ...
            accessor.`recast$activeFrame`(active)
            accessor.`recast$frameTicks`(remaining.toInt())
    ...
    fun observe() {
        ...
        for (sprite in sprites) {
            val accessor = sprite as? TextureAtlasSpriteAccessor ?: continue
            val animation = accessor.`recast$animation`() ?: continue
            uploaded[sprite] = animation.getFrameIndex(accessor.`recast$activeFrame`())
        }
    }
