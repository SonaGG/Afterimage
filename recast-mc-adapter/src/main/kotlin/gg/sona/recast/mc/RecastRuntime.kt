...
import gg.sona.recast.capture.recording.RecordingStore
import gg.sona.recast.clip.ClipStore
import gg.sona.recast.clip.export.ExportHandle
import gg.sona.recast.clip.export.ExportListener
...
import gg.sona.recast.clip.recording.compact.CompactJob
import gg.sona.recast.clip.recording.compact.RecordingCompactor
import gg.sona.recast.core.log.RecastLog
import gg.sona.recast.core.time.Nanos
import gg.sona.recast.editor.*
import gg.sona.recast.editor.host.*
import gg.sona.recast.editor.imgui.EditorContext
import gg.sona.recast.editor.imgui.PanelLog
...
class RecastRuntime(val minecraft: Minecraft) : EditorHost {

    private val logger = LogManager.getLogger("Recast")
    val platform = MinecraftPlatform(minecraft)
    val settings = RecastSettings(platform.root.resolve("recast.properties")).also { it.load() }
    ...
    override val replay: ReplayControl = object : ReplayControl {
        ...
        override fun describe(path: Path): RecordingInfo? = runCatching {
            RecastReader.open(path).use { reader ->
                val header = reader.header
                RecordingInfo(
                    reader.durationNanos,
                    header.startEpochMillis,
                    header.metadata(RecastFormat.MetadataKeys.PLAYER_NAME),
                    header.metadata(RecastFormat.MetadataKeys.SERVER_ADDRESS),
                    header.metadata(RecastFormat.MetadataKeys.TITLE),
                    Files.size(path),
                )
            }
        }.getOrNull()
        ...
        override fun rename(path: Path, name: String): Path? {
            ...
            val target = path.resolveSibling("$clean.${RecastFormat.EXTENSION}")
            ...
            return runCatching {
                ...
                val project = projectsDirectory.resolve(path.nameWithoutExtension + ".rcproj")
                if (Files.exists(project)) Files.move(project, projectsDirectory.resolve("$clean.rcproj"))
                target
            }.onFailure { logger.warn("Recast could not rename {}", path, it) }.getOrNull()
        }
        ...
        override fun openProject(path: Path): Boolean = this@RecastRuntime.openProject(path)

        override fun createProject(name: String, segments: List<Segment>): Path? = runCatching {
            val sessionId = segments.firstOrNull()?.let {
                runCatching {
                    RecastReader.open(it.gameplay).use { reader -> reader.header.sessionId }
                }.getOrNull()
            } ?: UUID.randomUUID()
            projects.create(name, segments, sessionId).file
        }.onFailure { logger.warn("Recast could not create project {}", name, it) }.getOrNull()

        override fun deleteProject(path: Path): Boolean = runCatching {
            val stitched =
                path.resolveSibling(path.fileName.toString().removeSuffix(ProjectStore.EXTENSION) + ".sequence.recast")
        ...
        override fun rebuildSequence(): Boolean {
            ...
            }.onFailure { logger.warn("Recast could not save project before rebuilding", it) }
            replayer.close()
    ...
    init {
        ...
        poseDriver.tickDelta = { (minecraft as MinecraftAccessor).`recast$timer`().partialTick }
    ...
    fun worldTick(boundaryNanos: Long) {
        ...
        try {
            ...
            logger.error("Recast world tick failed", error)
        } finally {
    ...
    fun onEntityCreated(entity: Entity) {
        ...
        (entity as? EntityAccessor)?.`recast$random`()?.setSeed(seed)
    }
    ...
    private fun reseed(seed: Long, entities: Boolean) {
        ...
        (minecraft.particleManager as? ParticleManagerAccessor)?.`recast$random`()?.setSeed(seed xor 1L)
        (minecraft.gameRenderer as? GameRendererAccessor)?.`recast$random`()?.setSeed(seed xor 2L)
        (minecraft.gui as? GameGuiAccessor)?.`recast$random`()?.setSeed(seed xor 3L)
        minecraft.textRenderer.random.setSeed(seed xor 4L)
        (minecraft.getEntityRenderDispatcher()
            .getRenderer<EndermanEntity>(EndermanEntity::class.java) as? EndermanRendererAccessor)?.`recast$random`()
            ?.setSeed(seed xor 5L)
        EnchantingTableBlockEntityAccessor.`recast$random`()?.setSeed(seed xor 6L)
        SpellParticleAccessor.`recast$random`()?.setSeed(seed xor 7L)
        displayRandom.setSeed(seed xor 8L)
        if (entities) {
            for (entity in world.entities) {
                (entity as? EntityAccessor)?.`recast$random`()
    ...
    private var autoOpenTicks = if (System.getenv("RECAST_AUTO_OPEN").isNullOrBlank()) -1 else 0
    ...
    private fun autoOpen() {
        ...
        autoOpenTicks = -1
        val requested = System.getenv("RECAST_AUTO_OPEN") ?: return
    ...
    private fun onReplayOpened(path: Path) {
        ...
        Thread({
            runCatching {
                ...
                .onFailure { logger.warn("Recast could not index events for {}", path.fileName, it) }
        }, "recast-events").apply { isDaemon = true }.start()
    }

    private fun openDropped(files: List<String>) {
        val recording = files.map { Paths.get(it) }
            .firstOrNull { it.fileName.toString().endsWith(".recast") && Files.isRegularFile(it) }
        if (recording == null) {
            message("Drop a .recast file to open it")
            return
        }
        val target = if (recording.toAbsolutePath().parent == recordings.directory.toAbsolutePath()) recording else {
            ...
            }.onFailure { logger.warn("Recast could not import {}", recording, it) }
            copy
        }
    ...
    private fun projectPath(recording: Path): Path =
        platform.projectsDirectory.resolve("${recording.nameWithoutExtension}.rcproj")

    private fun hudMessage(message: String) {
        platform.minecraft.player?.addMessage(LiteralText("[Recast] $message"))
        logger.info(message)
    }
