...
import gg.sona.recast.camera.impl.*
...
import gg.sona.recast.replay.state.interpolation.SnapInterpolation
...
class CameraDriver(private val minecraft: Minecraft, picker: ViewportPicker) : CameraControl {
    override val settings = CameraSettings()
    val scene = SceneCamera(minecraft, settings, picker)
    ...
    @Volatile
    var exportPose: CameraPose? = null
    var previewPose: CameraPose? = null
    var exporting: Boolean = false

    @Volatile
    var exportFov: Float = 0f

    @Volatile
    var exportOrthoScale: Float? = null
    ...
    fun exportPoseAt(nanos: Long): CameraPose {
        if (session == null) return lastPose
        pathProvider(nanos)?.let { return withShake(it, nanos) }
        if (settings.mode == CameraMode.FIRST_PERSON && settings.targetsRecorder()) recorderPose.poseAt(nanos)
            ?.let { return it }
        return currentPose()
    }
    ...
    fun onFrame(
        ...
        if (exactRecorderView && placeExactRecorderView(replay)) return
        if (settings.mode == CameraMode.FREE) {
    ...
    fun shouldHide(entity: Entity): Boolean = hiddenEntityId != Int.MIN_VALUE && entity.networkId == hiddenEntityId
