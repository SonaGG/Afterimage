    ...
    fun capture(minecraft: Minecraft): LocalScreen {
        ...
        return when (screen) {
            is CreativeInventoryScreen -> {
                ...
                val search = accessor.`recast$searchField`()?.text ?: ""
                screen(
                    LocalScreen.CREATIVE,
                    0,
                    search,
                    (accessor.`recast$scrollPosition`() * SCROLL_SCALE).toInt(),
                    screen.selectedTab
                )
            }
            ...
            is AnvilScreen -> {
                val field = (screen as AnvilScreenAccessor).`recast$renameField`()
                screen(
            ...
            is VillagerScreen -> screen(
                ...
                "",
                0,
                (screen as VillagerScreenAccessor).`recast$currentPage`()
            )

            is InventoryMenuScreen -> screen(LocalScreen.CONTAINER, screen.menu.networkId and 0xFF)
            is ChatScreen -> {
                val field = (screen as ChatScreenAccessor).`recast$chatField`()
                val text = field?.text ?: ""
            ...
            is SignEditScreen -> {
                ...
                val sign = accessor.`recast$sign`()
                val lines = sign.lines.joinToString("\n") { line: Text? -> line?.string ?: "" }
                screen(LocalScreen.SIGN, 0, lines, accessor.`recast$row`(), 0, sign.pos.toLong())
            }

            is BookEditScreen -> {
                val accessor = screen as BookEditScreenAccessor
                val page = accessor.`recast$currentPage`()
                val pages = accessor.`recast$pages`()
                val text = if (accessor.`recast$signing`()) accessor.`recast$title`() else pages?.let {
                    if (page in 0 until it.size()) it.getString(page) else ""
                } ?: ""
                screen(LocalScreen.BOOK, 0, text, if (accessor.`recast$signing`()) 1 else 0, page)
            }
