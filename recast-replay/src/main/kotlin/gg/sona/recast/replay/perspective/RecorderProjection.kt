...
class RecorderProjection(
    ...
    private fun inbound(packet: CapturedPacket, mode: DeliveryMode) {
        if (RecastInternal.isInternal(packet.packetId)) {
            if (packet.packetId == RecastInternal.OVERLAY_RESET) forward(packet, mode)
            return
        }
    ...
    private fun spawnRecorder(reference: CapturedPacket, mode: DeliveryMode) {
        ...
        val uuid = player.uuid ?: UUID.nameUUIDFromBytes("OfflinePlayer:${player.name ?: "Recast"}".toByteArray())
        val listed = shadow.players.entries.containsKey(uuid)
        val profile = shadow.players.profile(uuid)?.toPacketEntry()
            ?: PlayerListEntry(uuid, player.name ?: "Recast", emptyList(), player.gameMode, 0, null)
