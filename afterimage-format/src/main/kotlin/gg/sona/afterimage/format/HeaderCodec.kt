...
object HeaderCodec {
    ...
    fun encodeFileHeader(header: RecordingHeader): ByteArray {
        ...
        writer.writeBytes(AfterimageFormat.FILE_MAGIC)
        writer.writeShort(AfterimageFormat.VERSION)
    ...
    fun decodeFileHeader(reader: PacketReader): RecordingHeader {
        ...
        if (!magic.contentEquals(AfterimageFormat.FILE_MAGIC)) throw AfterimageFormatException("not a afterimage recording")
        val version = reader.readUnsignedShort()
        if (version > AfterimageFormat.VERSION) throw AfterimageFormatException("recording format $version is newer than supported ${AfterimageFormat.VERSION}")
    ...
    fun encodeSegmentHeader(
        ...
        val writer = PacketWriter(AfterimageFormat.SEGMENT_HEADER_BYTES)
        writer.writeBytes(AfterimageFormat.SEGMENT_MAGIC)
    ...
    fun decodeSegmentHeader(reader: PacketReader): SegmentHeader? {
        if (reader.remaining < AfterimageFormat.SEGMENT_HEADER_BYTES) return null
        val magic = reader.readBytes(4)
        if (!magic.contentEquals(AfterimageFormat.SEGMENT_MAGIC)) return null
    ...
    fun encodeIndex(segments: List<SegmentInfo>, blobs: List<BlobRef>, indexOffset: Long): ByteArray {
        val writer =
            PacketWriter(24 + segments.size * AfterimageFormat.INDEX_ENTRY_BYTES + blobs.size * AfterimageFormat.BLOB_ENTRY_BYTES + AfterimageFormat.TRAILER_BYTES)
        writer.writeBytes(AfterimageFormat.INDEX_MAGIC)
        ...
        writer.writeBytes(AfterimageFormat.BLOB_MAGIC)
        ...
        writer.writeBytes(AfterimageFormat.TRAILER_MAGIC)
        return writer.toByteArray()
    }
