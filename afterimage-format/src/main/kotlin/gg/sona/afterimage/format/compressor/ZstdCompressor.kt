...
class ZstdCompressor(private val level: Int = 3) : SegmentCompressor {
    ...
    override fun compress(source: ByteArray, length: Int): ByteArray {
        ...
        if (Zstd.isError(produced)) throw AfterimageFormatException("zstd compression failed: ${Zstd.getErrorName(produced)}")
    ...
    override fun decompress(source: ByteArray, rawLength: Int): ByteArray {
        ...
        if (Zstd.isError(produced)) throw AfterimageFormatException("zstd decompression failed: ${Zstd.getErrorName(produced)}")
        if (produced.toInt() != rawLength) throw AfterimageFormatException("zstd segment produced $produced of $rawLength bytes")
        return output
    }
}
