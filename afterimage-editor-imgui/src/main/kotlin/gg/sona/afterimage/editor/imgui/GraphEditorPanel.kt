...
class GraphEditorPanel(private val context: EditorContext) :
        ...
        if (Widgets.popupButton("graph-extrap", label, EditorFonts.px(172f), muted = target == null)) {
            if (target == null) Widgets.mutedText("Select a keyframe to pick a track")
                ...
                for (option in Extrapolation.entries) {
                    if (Menus.item(option.label, "", pre == option)) session.execute(
                ...
                for (option in Extrapolation.entries) {
                    if (Menus.item(option.label, "", post == option)) session.execute(
                        SetTrackExtrapolation(
                            lane,
                            pre,
                            option
    ...
    private fun contextMenu(session: EditorSession) {
        ...
        if (hit != null && hit.handleOut == null) {
            ...
            ImGui.separator()
            if (Menus.item("Go to keyframe")) session.replay?.seek(hit.key.timeNanos)
            ...
            if (ImGui.beginMenu("Interpolation")) {
                for (option in SegmentMode.entries) {
                    if (Menus.item(option.label, "", hit.key.mode == option)) {
                        val commands = ArrayList<EditorCommand>()
            ...
            if (Menus.item("Easy ease", "F9")) session.execute(
                EaseKeyframes.easyEase(
                    current.keyframeTimes,
                    current.valueKeys
                )
            )
            if (Menus.item("Ease in", "Shift+F9")) session.execute(
            ...
            if (Menus.item("Linear")) session.execute(
        ...
        if (channel != null && Menus.item("Add ${channel.label.lowercase()} keyframe here")) channel.insertKey(
            session,
            snapTime(contextNanos)
        )
        if (Menus.item("Fit everything")) fitAll(session)
        if (Menus.item("Fit selection", "F")) fitSelection(session)
        ImGui.separator()
        if (Menus.item("Value graph", "", mode == Mode.VALUE)) {
            mode = Mode.VALUE
            fitRequested = true
        }
        if (Menus.item("Speed graph", "", mode == Mode.SPEED)) {
            mode = Mode.SPEED
            fitRequested = true
        }
        if (Menus.item("Normalize curves", "", normalized)) {
            normalized = !normalized
            fitRequested = true
        }
        if (Menus.item("Show all channels")) {
            hidden.clear()
            solo = null
        }
    }
