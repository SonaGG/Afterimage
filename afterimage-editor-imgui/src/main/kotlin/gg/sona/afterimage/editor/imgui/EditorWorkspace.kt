...
import gg.sona.afterimage.clip.recording.trim.RecordingTrimmer
import gg.sona.afterimage.clip.recording.trim.TrimJob
...
class EditorWorkspace(val context: EditorContext) {
    ...
    private fun dockHost(): Int {
        ...
        ImGui.begin("##afterimage-dock-host", flags)
        ImGui.popStyleVar(3)
        val id = ImGui.getID("afterimage-dockspace")
    ...
    fun targetMenu(session: EditorSession) {
        ...
        for ((name, id) in players) {
            if (Menus.item(name, "", settings.targetEntityId == id)) {
    ...
    private fun menuBar() {
        ...
        if (ImGui.beginMenu("File")) {
            if (Menus.item("Save Project", "Ctrl+S", false, session != null)) saveProject()
            if (Menus.item("Export Video", "Ctrl+E", false, session != null)) context.openPanel("Export")
            if (Menus.item(
                    "Screenshot",
                    "F2",
                    false,
            ...
            if (Menus.item("Add Gameplay to Project", "", false, session != null)) context.openPanel("Sequence")
            if (Menus.item("Duplicate as Cut", "", false, session != null)) session?.let { duplicateAsCut(it) }
            Widgets.tooltip("A second project on the same gameplay: change cameras and clips here without touching the original")
            if (Menus.item("Trim Segment to In and Out", "", false, session != null)) trimSegmentToInOut()
            ImGui.separator()
            if (Menus.item(
                    "Close Replay",
                    "",
        ...
        if (ImGui.beginMenu("Edit")) {
            ...
            if (Menus.item("Delete", "Del", false, session?.selection?.isEmpty == false)) session?.deleteSelection()
            ImGui.separator()
            if (Menus.item("Select All Keyframes", "Ctrl+A", false, session != null)) session?.let {
                it.selection = Selection(keyframeTimes = it.project.camera.keyframeTimes().toSet())
            }
            if (Menus.item("Deselect", "Esc", false, session != null)) {
                session?.selection = Selection.NONE
                context.selectEntity(null)
            }
            ImGui.separator()
            if (Menus.item("Command Palette", "Ctrl+P")) palette.show()
            ImGui.endMenu()
        }
        if (ImGui.beginMenu("Playback")) {
            if (Menus.item("Play or Pause", "Space", false, replay != null)) replay?.togglePlaying()
            if (Menus.item("Play from In Point", "Shift+Space", false, replay != null)) replay?.let {
                it.seek(
                    session.project.inPointNanos
                ); it.play()
            }
            if (Menus.item("Shuttle Reverse", "J", false, replay != null)) shuttle(-1)
            if (Menus.item("Shuttle Forward", "L", false, replay != null)) shuttle(1)
            ImGui.separator()
            if (Menus.item(
                    "Step Frame Back",
                    "Left",
            ...
            if (Menus.item("Step Tick Back", "Shift+Left", false, replay != null)) replay?.stepTicks(-1)
            if (Menus.item("Step Tick Forward", "Shift+Right", false, replay != null)) replay?.stepTicks(1)
            if (Menus.item("Jump to Start", "Home", false, replay != null)) replay?.seek(0L)
            if (Menus.item("Jump to End", "End", false, replay != null)) replay?.seek(replay.endNanos)
            ImGui.separator()
            if (Menus.item(
                    "Set In Point",
                    "I",
            ...
            if (Menus.item("Set Out Point", "O", false, replay != null)) replay?.let {
                session.execute(
                    SetInOutPoints(minOf(session.project.inPointNanos, it.positionNanos), it.positionNanos)
                )
            }
            if (Menus.item("Clear In and Out", "", false, replay != null)) session?.execute(SetInOutPoints(0L, 0L))
            if (Menus.item("Loop In to Out", "", context.loopPlayback, replay != null)) context.loopPlayback =
                !context.loopPlayback
            ImGui.separator()
            if (Menus.item("Add Marker", "M", false, replay != null)) replay?.let {
                addMarker(
                    session,
                    it.positionNanos
                )
        ...
        if (ImGui.beginMenu("Camera")) {
            ...
            for (mode in CameraMode.entries) {
                if (Menus.item(mode.label, "", control.settings.mode == mode)) {
            ...
            if (Menus.item("Previous Keyframe", ",", false, session != null)) jumpKeyframe(-1)
            if (Menus.item("Next Keyframe", ".", false, session != null)) jumpKeyframe(1)
            ImGui.separator()
            if (Menus.item("Frame Selection", "F", false, session != null)) frameSelection()
            if (Menus.item("Snap to Recorder", "", false, replay != null)) snapToRecorder()
            ImGui.endMenu()
        }
        if (ImGui.beginMenu("View")) {
            for (tool in SceneTool.entries) if (Menus.item(
                    "${tool.label} Tool",
                    tool.key,
                    context.tool == tool
                )
            ) context.tool = tool
            if (Menus.item(if (context.localSpace) "Local Axes" else "World Axes", "X")) context.localSpace =
                !context.localSpace
            ImGui.separator()
            if (Menus.item("Fullscreen Scene", "Tab", fullscreen)) fullscreen = !fullscreen
            if (Menus.item(
                    "Camera Path",
                    "H",
            ...
            if (Menus.item("Scene Gizmos", "", context.ui.sceneGizmos)) context.ui.sceneGizmos =
                !context.ui.sceneGizmos
            if (Menus.item("Camera Preview", "", context.ui.cameraPreview)) context.ui.cameraPreview =
                !context.ui.cameraPreview
            if (Menus.item("Orientation Gizmo", "", context.ui.orientationGizmo)) context.ui.orientationGizmo =
                !context.ui.orientationGizmo
            if (Menus.item("Entity Outlines", "", context.ui.entityBoxes)) context.ui.entityBoxes =
                !context.ui.entityBoxes
            if (Menus.item("Keyframe Labels", "", context.ui.keyframeLabels)) context.ui.keyframeLabels =
                !context.ui.keyframeLabels
            if (Menus.item("Scene Status", "", context.ui.viewportOverlay)) context.ui.viewportOverlay =
                !context.ui.viewportOverlay
            ImGui.separator()
            if (Menus.item("Center Guide", "", context.visuals.centerGuide)) context.visuals.centerGuide =
                !context.visuals.centerGuide
            if (Menus.item("Rule of Thirds", "", context.visuals.thirdsGuide)) context.visuals.thirdsGuide =
            ...
            if (Menus.item("Reset Layout")) resetLayoutRequested = true
            ImGui.endMenu()
        }
        if (ImGui.beginMenu("Window")) {
            for (panel in panels) {
                if (Menus.item(panel.title, "", panel.open.get())) togglePanel(panel.title)
            }
            ImGui.endMenu()
        }
        if (ImGui.beginMenu("Help")) {
            ...
            ImGui.separator()
            if (Menus.item("About Afterimage")) {
                closeDialogs()
                aboutOpen.set(true)
            }
            ImGui.endMenu()
            ...
            if (ImGui.begin("##statusbar", flags)) {
                ...
                EditorFonts.with(EditorFonts.small) {
                    ...
                    Widgets.hintLine(
                        ...
                        if (messageActive) EditorTheme.TEXT.u32 else EditorTheme.TEXT_MUTED.u32
                    )
                    if (session != null) {
                        val parts = ArrayList<String>(4)
                        parts += if (session.project.dirty) (if (context.ui.autosave) "Autosaving" else "Unsaved") else "Saved"
                        val width = Widgets.hintLineWidth(parts)
    ...
    private fun shortcuts() {
        ...
        if (fullscreen && ImGui.isKeyPressed(ImGuiKey.Escape, false)) {
            fullscreen = false
            return
        }
    ...
    private fun trimRecording() {
        ...
        val output =
            source.resolveSibling("$base-trim-${TimeFormat.clock(start).replace(':', '-').substringBefore('.')}.afterimage")
