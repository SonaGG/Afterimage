...
import gg.sona.afterimage.editor.ProjectStore
import gg.sona.afterimage.editor.ProjectSummary
...
class LibraryScreen(private val context: EditorContext) {
    ...
    private fun sidebar() {
        ...
        val markHeight = EditorFonts.px(22f)
        val marked = Brand.mark(context.host, list, x + EditorFonts.px(14f), y + EditorFonts.px(12f), markHeight)
        ImGui.setCursorPos(if (marked) EditorFonts.px(14f) + Brand.markWidth(markHeight) + EditorFonts.px(8f) else EditorFonts.px(16f), EditorFonts.px(14f))
        EditorFonts.with(EditorFonts.heading) { ImGui.textUnformatted(Brand.NAME) }
    ...
    private fun sortPopup() {
        ...
        if (Widgets.beginPopup("lib-sort-menu")) {
            for (option in Sort.entries) if (Menus.item(option.label, "", sort == option)) sort = option
            Widgets.endPopup()
        }
    }
    ...
    private fun cardMenu(card: Card) {
        ...
        if (Menus.item("Open", "Enter")) open(card.path)
        if (Menus.item("Rename", "F2")) startRename(card)
        if (Menus.item("New Project from This")) newProject(listOf(card.path))
        ImGui.separator()
        if (Menus.item("Show in Folder")) openFolder(card.path)
        if (Menus.item("Copy Path")) ImGui.setClipboardText(card.path.toAbsolutePath().toString())
        if (Menus.item("Compact File")) {
        ...
        ImGui.separator()
        if (Menus.item("Delete", "Del")) confirmDelete = card.path
        Widgets.endPopup()
    }
    ...
    private fun projectMenu(summary: ProjectSummary) {
        ...
        if (Menus.item("Open", "Enter")) openProject(summary.path)
        if (Menus.item("Show in Folder")) openFolder(summary.path)
        ImGui.separator()
        if (Menus.item("Delete", "Del")) confirmDeleteProject = summary.path
        Widgets.endPopup()
    }
    ...
    private fun refresh() {
        ...
        cards = context.host.replay.listRecordings().map { path ->
            ...
            val project = ProjectStore.forRecording(projects, path)
