...
object ChunkBlobs {
    ...
    fun isRef(packet: CapturedPacket): Boolean =
        packet.direction == PacketDirection.CLIENTBOUND && packet.packetId == RecastInternal.CHUNK_REF
    ...
    fun readBlobs(body: ByteArray, onBlob: (offset: Int, length: Int, blob: ChunkBlob) -> Unit) {
        ...
        while (reader.hasRemaining()) {
            ...
            val length = reader.readVarInt()
            if (length < 0 || length > reader.remaining) throw RecastFormatException("chunk blob length $length exceeds segment")
