...
class RecastReader private constructor(
    val path: Path,
    ...
    fun readSegmentBody(segment: SegmentInfo): ByteArray {
        val headerBytes = readAt(segment.offset, RecastFormat.SEGMENT_HEADER_BYTES)
        val parsed = HeaderCodec.decodeSegmentHeader(PacketReader(headerBytes, 0, headerBytes.size))
            ?: throw RecastFormatException("segment ${segment.index} header is corrupt")
        val stored = readAt(segment.bodyOffset, parsed.storedLength)
        if (HeaderCodec.crc32(stored) != parsed.crc) throw RecastFormatException("segment ${segment.index} failed its checksum")
    ...
    private fun readAt(offset: Long, length: Int): ByteArray {
        ...
        while (buffer.hasRemaining()) {
            ...
            if (read < 0) throw RecastFormatException("unexpected end of recording at $position")
            position += read
        }
        return bytes
    }
    ...
    companion object {
        ...
        fun open(path: Path): RecastReader {
            ...
            try {
                ...
                return RecastReader(path, channel, header, segments, indexed?.blobs, indexed == null)
        ...
        private fun readIndex(channel: FileChannel, size: Long, firstSegmentOffset: Long): IndexData? {
            if (size < firstSegmentOffset + RecastFormat.TRAILER_BYTES) return null
            val trailer = read(channel, size - RecastFormat.TRAILER_BYTES, RecastFormat.TRAILER_BYTES)
            val trailerReader = PacketReader(trailer, 0, trailer.size)
            val indexOffset = trailerReader.readLong()
            if (!trailerReader.readBytes(4).contentEquals(RecastFormat.TRAILER_MAGIC)) return null
            if (indexOffset < firstSegmentOffset || indexOffset >= size - RecastFormat.TRAILER_BYTES) return null
            val indexBytes = read(channel, indexOffset, (size - RecastFormat.TRAILER_BYTES - indexOffset).toInt())
            val reader = PacketReader(indexBytes, 0, indexBytes.size)
            if (!reader.readBytes(4).contentEquals(RecastFormat.INDEX_MAGIC)) return null
            val count = reader.readInt()
            if (count < 0 || reader.remaining < count * RecastFormat.INDEX_ENTRY_BYTES) return null
            ...
            for (index in 0 until count) {
                ...
                if (offset < firstSegmentOffset || offset + RecastFormat.SEGMENT_HEADER_BYTES + storedLength > indexOffset) return null
            ...
            if (reader.remaining >= 8 && reader.readBytes(4).contentEquals(RecastFormat.BLOB_MAGIC)) {
                val blobCount = reader.readInt()
                if (blobCount < 0 || reader.remaining < blobCount * RecastFormat.BLOB_ENTRY_BYTES) return null
                repeat(blobCount) {
                    blobs += BlobRef(
                        reader.readLong(),
                        reader.readInt(),
        ...
        private fun scanSegments(channel: FileChannel, size: Long, firstSegmentOffset: Long): List<SegmentInfo> {
            ...
            var packetIndex = 0
            while (offset + RecastFormat.SEGMENT_HEADER_BYTES <= size) {
                val headerBytes = read(channel, offset, RecastFormat.SEGMENT_HEADER_BYTES)
                ...
                val bodyOffset = offset + RecastFormat.SEGMENT_HEADER_BYTES
