...
object PacketRecordCodec {
    ...
    fun read(reader: PacketReader, previousNanos: Long): CapturedPacket {
        ...
        if (length < 0 || length > reader.remaining) throw RecastFormatException("packet record length $length exceeds segment")
    ...
    fun readAll(body: ByteArray, startNanos: Long, expectedCount: Int): List<CapturedPacket> {
        ...
        if (expectedCount >= 0 && packets.size != expectedCount) {
            throw RecastFormatException("segment declared $expectedCount packets but contained ${packets.size}")
        }
        return packets
    }
}
