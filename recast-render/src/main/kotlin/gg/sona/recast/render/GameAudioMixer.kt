...
object GameAudioMixer {
    ...
    fun muxArguments(
        ...
        val codec = when (format) {
            ExportFormat.WEBM_VP9 -> listOf("-c:a", "libopus", "-b:a", "160k")
            ExportFormat.MOV_PRORES -> listOf("-c:a", "pcm_s16le")
        ...
        if (existingAudio) {
            arguments += listOf(
                "-filter_complex",
                "[0:a:0][1:a:0]amix=inputs=2:duration=first:dropout_transition=0:normalize=0[a]",
                "-map",
                "0:v:0",
                "-map",
                "[a]"
            ...
            arguments += listOf("-map", "0:v:0", "-map", "1:a:0")
        }
        arguments += listOf("-c:v", "copy")
        arguments += codec
        arguments += listOf("-shortest")
        if (format == ExportFormat.MP4_H264 || format == ExportFormat.MP4_H265) arguments += listOf(
            "-movflags",
            "+faststart"
        )
