...
class FfmpegRuntime(root: Path) {
    private val logger = RecastLog.logger("recast.ffmpeg")
    private val build = FfmpegBuild.current()

    val directory: Path = System.getenv("RECAST_FFMPEG")?.takeIf { it.isNotBlank() }?.let { Paths.get(it) }
    ...
    fun load(): Boolean {
        ...
        try {
            ...
            logger.warn("Recast could not load ffmpeg from $directory", error)
            return false
        }
        return true
    }
