...
class CameraDriver(private val minecraft: Minecraft, private val picker: ViewportPicker) : CameraControl, ExportCamera {
    ...
    private fun cameraEntity(level: ClientLevel): ReplayCameraEntity =
        cameraEntity?.takeIf { it.level() === level } ?: ReplayCameraEntity(level).also { cameraEntity = it }
    ...
    fun hideHand(): Boolean {
        ...
        if (settings.targetsRecorder()) return session?.shadow?.localPlayer?.gameType == GameType.SPECTATOR
        return handTarget() == null
    }

    fun onSettleStart() {
        minecraft.player?.let { SwingStates.reset(it) }
    }
    ...
    fun onSeeked(replay: ReplaySession) {
        ...
        val player = minecraft.player ?: return
        val local = replay.shadow.localPlayer
        SwingStates.apply(player, local.swings, local.effects.values, replay.lastTickNanos)
    ...
    private fun mirrorHand(target: AbstractClientPlayer) {
        ...
        SwingStates.copy(target, player)
    ...
    fun onClientTick() {
        ...
        val player = minecraft.player ?: return
        val recorder = minecraft.level?.getEntity(replay.shadow.localPlayer.entityId) as? Player ?: return
    ...
    private fun targetSource(): PoseSource {
        ...
        return PoseSource { nanos ->
            ...
            val shadowEntity = session?.shadow?.entityMap?.get(entityId) ?: return@PoseSource null
