...
class RecorderProjection(private val shadow: ShadowClient, private val downstream: ReplayConsumer) : ReplayConsumer {
        ...
        if (AfterimageInternal.isInternal(packet.packetId)) {
            when (val internal = InternalCodec.decode(packet)) {
                ...
                is LocalHand -> if (internal.isSwing) {
                    nativeSwings = true
                    swing(ShadowSwings.handOf(internal), ShadowSwings.animationOf(internal), nanos, mode)
    ...
    private fun syncCameraInfo(nanos: Long, mode: DeliveryMode) {
        ...
        emit(ShadowPlayerList.addPacket(listOf(entry)), nanos, mode)
    }
    ...
    private fun spawnRecorder(nanos: Long, mode: DeliveryMode) {
        ...
        if (!listed) emit(ShadowPlayerList.addPacket(listOf(entry)), nanos, mode)
        emit(
