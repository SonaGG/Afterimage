...
import gg.sona.afterimage.mc263.mixin.ChatComponentInvoker
import gg.sona.afterimage.mc263.mixin.HudAccessor
import gg.sona.afterimage.mc263.mixin.SetPlayerTeamAccessor
...
import gg.sona.afterimage.mc263.state.Components
import gg.sona.afterimage.mc263.state.ShadowOverlays
import gg.sona.afterimage.mc263.state.ShadowPlayerList
import gg.sona.afterimage.mc263.state.ShadowScoreboard
import gg.sona.afterimage.mc263.state.ShadowSwings
import gg.sona.afterimage.mc263.state.ShadowTeam
...
class VirtualConnection(private val minecraft: Minecraft, private val profile: GameProfile) : ReplayConsumer {
    ...
    fun open() {
        ...
        ReplayConnections.register(connection)
    ...
    override fun onPacket(packet: CapturedPacket, mode: DeliveryMode) {
        ...
        if (mode == DeliveryMode.SEEK && McReplayProtocol.INSTANCE.isTransient(packet)) return
        val wireId = PacketIds.wireId(packet.packetId)
        if (!PacketIds.isConfiguration(packet.packetId)) {
            if (wireId == PacketIds.BUNDLE_DELIMITER || wireId == PacketIds.DELETE_CHAT) return
        }
        var payload = packet.payload
        if (!PacketIds.isConfiguration(packet.packetId)) {
            when (wireId) {
                PacketIds.LOGIN -> dropStalePlayer()
                PacketIds.PLAYER_INFO_REMOVE -> retainPlayerInfo(packet)
                PacketIds.FORGET_LEVEL_CHUNK -> if (chunkCenter() != null) return
                PacketIds.LEVEL_CHUNK_WITH_LIGHT -> chunkCenter()?.let { center ->
                ...
                PacketIds.SET_CHUNK_CACHE_CENTER -> chunkCenter()?.let { center -> payload = encode(ClientboundSetChunkCacheCenterPacket(center[0], center[1])) ?: payload }
                PacketIds.SET_CHUNK_CACHE_RADIUS -> chunkRadius()?.let { radius -> payload = encode(ClientboundSetChunkCacheRadiusPacket(radius)) ?: payload }
                PacketIds.SET_OBJECTIVE, PacketIds.SET_PLAYER_TEAM -> payload = sanitizeScoreboard(packet) ?: return
                PacketIds.PLAYER_CHAT -> payload = rewriteChat(packet) ?: return
                PacketIds.SET_TIME -> payload = rewriteTime(packet) ?: payload
            }
        }
        val ghost = if (wireId == PacketIds.ADD_ENTITY && !PacketIds.isConfiguration(packet.packetId)) ensureSpawnProfile(payload) else null
    ...
    private fun ensureSpawnProfile(payload: ByteArray): UUID? {
        ...
        try {
            ...
            deliverDecoded(ShadowPlayerList.addPacket(listOf(entry)))
            return uuid
    ...
    fun close() {
        ...
        connection?.let { ReplayConnections.unregister(it) }
    }
    ...
    private fun decode(packet: CapturedPacket): Packet<*>? {
        ...
        writeVarInt(buffer, PacketIds.wireId(packet.packetId))
    ...
    private fun sanitizeScoreboard(packet: CapturedPacket): ByteArray? {
        ...
        val replacement: Packet<*>? = when (decoded) {
            is ClientboundSetObjectivePacket -> {
                ...
                when (decoded.method) {
                    ClientboundSetObjectivePacket.METHOD_ADD -> if (existing == null) decoded else ClientboundSetObjectivePacket(ShadowScoreboard.dummyObjective(decoded), ClientboundSetObjectivePacket.METHOD_CHANGE)
                    ClientboundSetObjectivePacket.METHOD_REMOVE -> if (existing == null) null else decoded
                    ClientboundSetObjectivePacket.METHOD_CHANGE -> if (existing == null) ClientboundSetObjectivePacket(ShadowScoreboard.dummyObjective(decoded), ClientboundSetObjectivePacket.METHOD_ADD) else decoded
                    else -> decoded
                }
            }
            ...
            is ClientboundSetPlayerTeamPacket -> {
                ...
                when ((decoded as SetPlayerTeamAccessor).afterimage_method()) {
                    ShadowTeam.METHOD_ADD -> if (existing == null) decoded else {
                        val members = decoded.players.filter { scoreboard.getPlayersTeam(it)?.name != decoded.name }
                        deliverDecoded(SetPlayerTeamInvoker.afterimage_create(decoded.name, ShadowTeam.METHOD_CHANGE, decoded.parameters, emptyList()))
                        if (members.isEmpty()) null else SetPlayerTeamInvoker.afterimage_create(decoded.name, ShadowTeam.METHOD_JOIN, Optional.empty(), members)
                    }

                    ShadowTeam.METHOD_CHANGE -> if (existing == null) SetPlayerTeamInvoker.afterimage_create(decoded.name, ShadowTeam.METHOD_ADD, decoded.parameters, emptyList()) else decoded
                    ShadowTeam.METHOD_LEAVE -> {
                        val members = if (existing == null) emptyList() else decoded.players.filter { scoreboard.getPlayersTeam(it)?.name == decoded.name }
                        if (members.isEmpty()) null else SetPlayerTeamInvoker.afterimage_create(decoded.name, ShadowTeam.METHOD_LEAVE, Optional.empty(), members)
                    }
    ...
    private fun rewriteChat(packet: CapturedPacket): ByteArray? {
        ...
        return encode(ClientboundSystemChatPacket(ShadowOverlays.decorate(decoded), false))
    }
    ...
    private fun deliverDecoded(packet: Packet<*>) {
        ...
        val buffer = Unpooled.buffer(payload.size + 5)
        writeVarInt(buffer, PacketIds.clientbound(packet.type()))
        buffer.writeBytes(payload)
    ...
    private fun resetChat(reset: OverlayReset) {
        ...
        for (line in reset.chat.sortedByDescending { it.ageNanos }) {
            val text = Components.parse(line.json, registries) ?: continue
    ...
    private fun applyLocalHand(packet: LocalHand) {
        ...
        when (packet.action) {
            ...
            LocalHand.ITEM_USED -> player.itemUsed(ShadowSwings.handOf(packet))
        }
    }
