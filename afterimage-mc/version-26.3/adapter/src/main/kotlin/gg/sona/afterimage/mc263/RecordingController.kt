...
import gg.sona.afterimage.mc263.net.PacketTaps
import gg.sona.afterimage.mc263.protocol.McReplayProtocol
import gg.sona.afterimage.mc263.state.ShadowClient
...
import gg.sona.afterimage.mc263.mixin.ConnectionAccessor
...
class RecordingController(private val minecraft: Minecraft, private val store: RecordingStore, private val clips: ClipStore) {
    private class Live(val channel: Channel, val capture: PacketCapture, val state: ShadowClient, val deriver: EventDeriver, val engine: FlashbackEngine)
    ...
    val shadow: ShadowClient? get() = live?.state
    private val inventoryCapture = InventoryCapture(minecraft) { packet ->
        val session = live?.takeIf { it.capture.isRecording } ?: return@InventoryCapture Unit
        val encoded = session.state.codecs.encode(packet, 0L) ?: return@InventoryCapture Unit
    ...
    val camera = CameraCapture(minecraft).also { it.capture = { live?.capture } }
    ...
    fun onConnection(connection: Connection) {
        ...
        onDisconnect()
        val state = ShadowClient(identity())
        val capture = PacketCapture(CaptureConfig(keyframeIntervalNanos = keyframeIntervalNanos), { state }, protocolVersion = PacketIds.protocolVersion)
    ...
    private fun sampleScreen(session: Live, allowMouse: Boolean) {
        val current = ScreenCapture.capture(minecraft)
    ...
    fun startRecording(): Boolean {
        ...
        metadata[AfterimageFormat.MetadataKeys.MINECRAFT_VERSION] = McReplayProtocol.MINECRAFT_VERSION
