...
import gg.sona.afterimage.mc26.ScreenCapture26
import gg.sona.afterimage.mc26.mixin.BookEditScreenAccessor
import gg.sona.afterimage.mc26.mixin.ChatScreenAccessor
import gg.sona.afterimage.mc26.mixin.CreativeModeInventoryScreenAccessor
import gg.sona.afterimage.mc26.mixin.MultiPlayerGameModeAccessor
import gg.sona.afterimage.mc26.mixin.SignEditScreenAccessor
import gg.sona.afterimage.mc26.state.ShadowWindow26
...
class ScreenMirror26(private val minecraft: Minecraft, private val cameraDriver: CameraDriver26, private val session: () -> ReplaySession?) {
    ...
    fun chatFocused(): Boolean = active() && session()?.shadow26?.localPlayer?.screen?.kind == LocalScreen.CHAT

    fun recordedState(): LocalScreen? = if (active()) session()?.shadow26?.localPlayer?.screen else null

    fun modelEntity(fallback: LivingEntity): LivingEntity {
        if (!active()) return fallback
        val recorderId = session()?.shadow26?.localPlayer?.entityId ?: return fallback
    ...
    private fun ensure(state: LocalScreen, window: ShadowWindow26?, player: LocalPlayer, width: Int, height: Int): Screen? {
    ...
    private fun create(state: LocalScreen, window: ShadowWindow26?, player: LocalPlayer): Screen? = when (state.kind) {
    ...
    private fun containerScreen(window: ShadowWindow26, player: LocalPlayer): Screen? {
    ...
    private fun sync(target: Screen, state: LocalScreen, window: ShadowWindow26?) {
        when (target) {
    ...
    private fun syncCreative(target: CreativeModeInventoryScreen, state: LocalScreen) {
        ...
        withCreativeMode(target) {
            ...
            accessor.afterimage_setScrollOffs((state.cursor / ScreenCapture26.SCROLL_SCALE).coerceIn(0f, 1f))
        }
    }
