...
import gg.sona.afterimage.mc263.mixin.EnchantingTableBlockEntityAccessor
import gg.sona.afterimage.mc263.mixin.EntityAccessor
import gg.sona.afterimage.mc263.mixin.FontAccessor
...
import gg.sona.afterimage.mc263.mixin.KeyMappingAccessor
import gg.sona.afterimage.mc263.mixin.LightmapRenderStateExtractorAccessor
import gg.sona.afterimage.mc263.mixin.ParticleAccessor
import gg.sona.afterimage.mc263.mixin.ParticleEngineAccessor
import gg.sona.afterimage.mc263.mixin.SpellParticleAccessor
import gg.sona.afterimage.mc263.mixin.TimerAccessor
import gg.sona.afterimage.mc263.render.McExportPlatform
import gg.sona.afterimage.mc263.render.GizmoRenderer
import gg.sona.afterimage.mc263.render.LookPreview
import gg.sona.afterimage.mc263.render.PreviewRenderer
import gg.sona.afterimage.mc263.render.ViewportFit
import gg.sona.afterimage.mc263.replay.*
import gg.sona.afterimage.mc263.ui.ExportScreen
import gg.sona.afterimage.mc263.ui.WorkspaceOverlay
import gg.sona.afterimage.mc263.ui.McWorkspaceScreens
...
class AfterimageRuntime(val minecraft: Minecraft) : EditorHost {
    ...
    val recorder = RecordingController(minecraft, recordings, clips)
    ...
    val workspace = WorkspaceHost(McGfx.gfx, McWorkspaceScreens(minecraft), platform.root.resolve("afterimage-workspace.ini"))
    val keybinds = AfterimageKeybinds(minecraft)
    val ffmpeg = FfmpegRuntime(platform.root)
    val post = PostProcessor(McGfx.gfx, { platform.lutsDirectory }) { message, error -> logger.warn("(Afterimage) {}", message, error) }
    val exportPlatform = McExportPlatform(minecraft)
    val sounds = SoundCapture(minecraft).also { it.settling = { replayer.session?.settling == true } }
    ...
    val viewportFit = ViewportFit(minecraft)
    private val recorderSync = RecorderSync()
    ...
    val screenMirror = ScreenMirror(minecraft, cameraDriver) { replayer.session }.also { mirror -> cameraDriver.recordedHud = { mirror.recordedState() } }
    val preview = PreviewRenderer(minecraft, cameraDriver)
    val clock = ReplayClock()
    val seekRestorer = SeekRestorer(minecraft, exporter.sounds, clock, { beginTickEvents(it) }, { animateTick(it) }).also { replayer.seekRestorer = it }
    private val displayRandom = Random()

    val thumbnails = ThumbnailService(McGfx.gfx, McThumbnailHost(minecraft)) { workspace.currentWorldViewport() }
    ...
    init {
        SdlWindow.display = WindowDisplay(minecraft)
        ...
        exporter.onExportStarted = {
            clock.reset()
            minecraft.gui.setScreen(ExportScreen { cancelRunningExports() })
        }
        exporter.onExportFinished = {
            if (minecraft.gui.screen() is ExportScreen) gg.sona.afterimage.mc263.ui.ScreenGuard.allow { minecraft.gui.setScreen(null) }
        ...
        visualsController.mobTypeProvider = { id -> context.session?.replay?.shadow?.entityMap?.get(id)?.takeIf { it.kind == EntityKind.MOB }?.type }
        keybinds.register()
    }
    ...
    fun animateOrigin(): net.minecraft.world.entity.Entity? {
        ...
        val recorder = replayer.session?.shadow?.localPlayer?.entityId?.let { level.getEntity(it) }
    ...
    fun onEntitySpawned(id: Int) {
        ...
        val spawnedAt = replay.shadow.entityMap[id]?.spawnedAtNanos ?: eventNanos
    ...
    private fun reseed(seed: Long, entities: Boolean = false) {
        ...
        AfterimageHooks.reseedVisualRandoms(seed)
        if (entities) {
    ...
    fun onFrameStart(tickDelta: Float) {
        ...
        if (replayer.isReplaying && (exporter.isExporting || replayer.syncChunkFrames > 0)) minecraft.level?.let { LightSync.drain(it) }
        applyPackTrack()
    ...
    private fun lookPreviewRequest(): LookPreview.Request? {
        ...
        return LookPreview.Request(look, session.focusDistanceAt(replay.positionNanos, pose.position), false, viewportPicker.tanHalfFov(), (replay.positionNanos / PREVIEW_GRAIN_NANOS % 4096L).toFloat())
    }
    ...
    fun onFrameEnd() {
        ...
        if (exporter.isExporting) workspace.renderExport() else WorkspaceOverlay.drawAfterGui()
    ...
    private fun onReplayOpened(path: Path) {
        ...
        Thread({
            runCatching { FileReplaySource.open(path).use { editor.events.build(path, it, session.shadow.recorderIdentity) } }
