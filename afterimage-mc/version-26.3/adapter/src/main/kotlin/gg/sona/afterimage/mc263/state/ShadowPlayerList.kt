...
import gg.sona.afterimage.mc263.mixin.PlayerInfoUpdateAccessor
...
class ShadowPlayer(override val uuid: UUID, var entry: ClientboundPlayerInfoUpdatePacket.Entry) : PlayerProfile {
...
class ShadowPlayerList(val known: MutableMap<UUID, ShadowPlayer> = HashMap()) : PlayerListState {
    val entries = LinkedHashMap<UUID, ShadowPlayer>()
    var tabList: ClientboundTabListPacket? = null

    override val listed: Collection<PlayerProfile> get() = entries.values

    override fun profile(uuid: UUID): ShadowPlayer? = entries[uuid] ?: known[uuid]

    override fun knownProfile(uuid: UUID): ShadowPlayer? = known[uuid]
    ...
    fun apply(packet: ClientboundPlayerInfoUpdatePacket) {
        ...
        for (update in packet.entries()) {
            ...
            if (existing == null) {
                ...
                val player = ShadowPlayer(uuid, update)
                entries[uuid] = player
                known[uuid] = player
                continue
            }
...
class ShadowObjective(val packet: ClientboundSetObjectivePacket) : ObjectiveState {
...
class ShadowTeam(override val name: String) : TeamState {
...
class ShadowScoreboard : ScoreboardState {
    val objectives = LinkedHashMap<String, ShadowObjective>()
    val teams = LinkedHashMap<String, ShadowTeam>()
    ...
    fun apply(packet: ClientboundSetObjectivePacket) {
        when (packet.method) {
            ClientboundSetObjectivePacket.METHOD_ADD -> objectives[packet.objectiveName] = ShadowObjective(packet)
            ...
            ClientboundSetObjectivePacket.METHOD_CHANGE -> objectives[packet.objectiveName]?.let { existing ->
                val replacement = ShadowObjective(packet)
    ...
    fun apply(packet: ClientboundSetPlayerTeamPacket) {
        ...
        when (method) {
            ShadowTeam.METHOD_ADD -> {
                val team = ShadowTeam(packet.name)
            ...
            ShadowTeam.METHOD_REMOVE -> teams.remove(packet.name)
            ShadowTeam.METHOD_CHANGE -> teams[packet.name]?.let { it.parameters = packet.parameters.orElse(it.parameters) }
            ShadowTeam.METHOD_JOIN -> teams[packet.name]?.let { team ->
                for (other in teams.values) if (other !== team) other.members.removeAll(packet.players.toSet())
                team.members.addAll(packet.players)
            }

            ShadowTeam.METHOD_LEAVE -> teams[packet.name]?.members?.removeAll(packet.players.toSet())
        }
    }
    ...
    companion object {
        ...
        fun objectivePacket(objective: ShadowObjective, method: Int): ClientboundSetObjectivePacket =
