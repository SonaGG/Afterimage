...
import gg.sona.afterimage.mc263.protocol.ProtocolSupport
import gg.sona.afterimage.mc263.replay.RecorderProjection
import gg.sona.afterimage.mc263.replay.ReplayConnections
import gg.sona.afterimage.mc263.ui.ExportScreen
import gg.sona.afterimage.mc263.ui.WorkspaceScreen
...
object AfterimageHooks {
    private val logger = LoggerFactory.getLogger("Afterimage")

    @Volatile
    private var runtime: AfterimageRuntime? = null

    fun install() {
        ProtocolSupport.install()
    }

    private fun runtime(): AfterimageRuntime? {
        runtime?.let { return it }
        val minecraft = Minecraft.getInstance() ?: return null
        if (!minecraft.isSameThread) return null
        return try {
            AfterimageRuntime(minecraft).also { runtime = it }
    ...
    val current: AfterimageRuntime? get() = runtime

    @JvmStatic
    fun onConnection(connection: Connection) = runTask("connection") {
        if (!ReplayConnections.isReplay(connection)) it.recorder.onConnection(connection)
    }

    @JvmStatic
    fun onPlayConnection(connection: Connection) = runTask("play connection") {
        if (ReplayConnections.isReplay(connection)) return@runTask
    ...
    fun keepsReplayScreen(requested: Screen?): Boolean {
        val current = runtime ?: return false
        if (!current.replayer.isReplaying) return false
        if (gg.sona.afterimage.mc263.ui.ScreenGuard.closing) return false
        val blocked = requested !is WorkspaceScreen && requested !is ExportScreen
    ...
    fun shouldHideEntity(entity: Entity): Boolean {
        ...
        if (current.replayer.isReplaying && (entity === current.minecraft.player || RecorderProjection.isCamera(entity))) return true
    ...
    fun onSoundPlay(engine: SoundEngine, sound: SoundInstance): Boolean {
        val current = runtime ?: return false
        return (current.exporter.sounds as gg.sona.afterimage.mc263.replay.SoundCapture).onPlay(engine, sound)
    }
    ...
    fun onReplayConnectionError(connection: Connection, error: Throwable): Boolean {
        if (!ReplayConnections.isReplay(connection)) return false
