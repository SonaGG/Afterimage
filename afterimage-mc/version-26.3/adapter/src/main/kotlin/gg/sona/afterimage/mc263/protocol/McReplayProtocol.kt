...
class McReplayProtocol : ReplayProtocol {
    override val version: Int get() = PacketIds.protocolVersion
    ...
    override fun isJoin(packet: CapturedPacket): Boolean =
        packet.direction == PacketDirection.CLIENTBOUND && packet.packetId == PacketIds.LOGIN
    ...
    override fun respawnDimension(packet: CapturedPacket): Int? {
        if (packet.direction != PacketDirection.CLIENTBOUND || packet.packetId != PacketIds.RESPAWN) return null
    ...
    override fun isTransient(packet: CapturedPacket): Boolean {
        ...
        return packet.packetId in PacketIds.TRANSIENT
    }

    override fun isVisualEvent(packet: CapturedPacket): Boolean {
        if (packet.direction != PacketDirection.CLIENTBOUND) return packet.packetId == AfterimageInternal.LOCAL_BLOCK_BREAK
        return packet.packetId in PacketIds.VISUAL_EVENTS
    }
