...
import gg.sona.afterimage.mc263.mixin.MoveEntityAccessor
import gg.sona.afterimage.mc263.mixin.RotateHeadAccessor
...
class ShadowClient(override val identity: RecorderIdentity, knownProfiles: MutableMap<UUID, ShadowPlayer> = HashMap()) : ReplayState, WorldState, EntityStates {
    val codecs = Codecs()
    val configuration = ArrayList<CapturedPacket>()
    val level = ShadowWorld()
    val entityMap = IntObjectMap<ShadowEntity>(256)
    override val players = ShadowPlayerList(knownProfiles)
    override val scoreboard = ShadowScoreboard()
    val overlays = ShadowOverlays()
    override val localPlayer = ShadowLocalPlayer(identity)
    ...
    override fun dimensionName(dimension: Int): String = McReplayProtocol.dimensionName(dimension)
    ...
    override fun apply(packet: CapturedPacket, events: WorldEventSink?) {
        ...
        if (events != null) PacketEvents.before(decoded, this, nanos, events)
        when (decoded) {
        ...
        if (events != null) PacketEvents.after(decoded, this, nanos, events)
    }
    ...
    private fun applyInternal(packet: ServerboundPacket, nanos: Long, events: WorldEventSink?) {
        ...
        if (events != null) PacketEvents.internal(packet, nanos, events)
    }
    ...
    fun applyDecoded(source: CapturedPacket, packet: Packet<*>, nanos: Long) {
        when (packet) {
            ...
            is ClientboundAddEntityPacket -> entityMap.put(packet.id, ShadowEntity(packet, nanos, Retained.hashOf(source)))
            ...
            is ClientboundUpdateMobEffectPacket -> {
                ...
                target?.put(packet.effect, ShadowEffect(packet, nanos))
            }
            ...
            is ClientboundOpenScreenPacket -> window = ShadowWindow(packet.containerId, packet, null, nanos)
            is ClientboundMountScreenOpenPacket -> window = ShadowWindow(packet.containerId, null, packet, nanos)
    ...
    private fun setDimension(info: CommonPlayerSpawnInfo) {
        ...
        level.dimension = McReplayProtocol.dimensionId(level.dimensionKey)
    ...
    override fun fork(): ReplayState = ShadowClient(recorderIdentity, players.known)

    override fun canDiff(target: ReplayState): Boolean = target is ShadowClient && StateDiff.canDiff(this, target)

    override fun diff(target: ReplayState, nanos: Long): List<CapturedPacket> = StateDiff.packets(this, target as ShadowClient, nanos)
    ...
    override fun diff(target: ReplayState, nanos: Long, freshEntities: Boolean): List<CapturedPacket> {
        ...
        return StateDiff.packets(this, target as ShadowClient, nanos, freshEntities)
    }
