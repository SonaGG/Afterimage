...
class ReplayController(private val platform: MinecraftPlatform, val camera: CameraDriver) {
    ...
    fun open(target: Path): Boolean {
        ...
        return try {
            ...
            val mirror = VirtualConnection(minecraft, cameraProfile(shadow))
            ...
            val projection = RecorderProjection(shadow, mirror)
    ...
    private fun cameraProfile(shadow: ShadowClient): GameProfile {
        val identity = shadow.recorderIdentity
        val uuid = identity.uuid
        val name = identity.name
        if (uuid != null && !name.isNullOrEmpty()) return GameProfile(RecorderProjection.cameraUuid(uuid), name)
        val user = minecraft.user
        return GameProfile(RecorderProjection.cameraUuid(user.profileId), user.name)
    }
    ...
    private fun close(leaveWorld: Boolean) {
        ...
        if (leaveWorld) {
            ...
            if (minecraft.gui.screen() !is TitleScreen) ScreenGuard.allow { minecraft.gui.setScreen(TitleScreen()) }
        } else {
    ...
    fun ownsWorld(): Boolean {
        ...
        return ReplayConnections.isReplay(connection)
    }
    ...
    private val chunkWindow = ChunkWindow(minecraft)
    ...
    private fun advanceToFirstPosition(replay: ReplaySession, shadow: ShadowClient) {
