...
class TitleState(val titleJson: String?, val subtitleJson: String?, val fadeIn: Int, val stay: Int, val fadeOut: Int, val shownAtNanos: Long) {
...
class ShadowOverlays {
    ...
    fun visibleChat(nanos: Long, windowNanos: Long = CHAT_VISIBLE_NANOS): List<ChatLine> = chat.filter { it.nanos <= nanos && nanos - it.nanos <= windowNanos }

    fun visibleActionBar(nanos: Long): ChatLine? = actionBar?.takeIf { it.nanos <= nanos && nanos - it.nanos <= ACTION_BAR_NANOS }
    ...
    fun apply(packet: Packet<*>, nanos: Long, registries: HolderLookup.Provider?): Boolean {
        when (packet) {
            ...
            is ClientboundSetTitleTextPacket -> {
                titleJson = Components.json(packet.text(), registries)
                titleShownAt = nanos
            }

            is ClientboundSetSubtitleTextPacket -> subtitleJson = Components.json(packet.text(), registries)
    ...
    fun apply(packet: OverlayReset, nanos: Long) {
        if (packet.resetsChat) {
            chat.clear()
            for ((ageNanos, json) in packet.chat.sortedByDescending { it.ageNanos }) chat.addLast(ChatLine(nanos - ageNanos,
                json
            ))
        }
    ...
    fun snapshot(nanos: Long, registries: HolderLookup.Provider?, vanilla: MutableList<Packet<*>>): OverlayReset {
        ...
        bar?.let { line -> Components.parse(line.json, registries)?.let { vanilla += ClientboundSetActionBarTextPacket(it) } }
        if (titleActive) {
            vanilla += ClientboundSetTitlesAnimationPacket(current.fadeIn, current.stay, current.fadeOut)
            vanilla += ClientboundSetSubtitleTextPacket(current.subtitleJson?.let { Components.parse(it, registries) } ?: Component.empty())
            current.titleJson?.let { json -> Components.parse(json, registries)?.let { vanilla += ClientboundSetTitleTextPacket(it) } }
        }
        return OverlayReset(
    ...
    private fun line(component: Component, nanos: Long, registries: HolderLookup.Provider?) {
        chat.addLast(ChatLine(nanos, Components.json(component, registries)))
    ...
    private fun actionBar(component: Component, nanos: Long, registries: HolderLookup.Provider?) {
        actionBar = ChatLine(nanos, Components.json(component, registries))
    }
