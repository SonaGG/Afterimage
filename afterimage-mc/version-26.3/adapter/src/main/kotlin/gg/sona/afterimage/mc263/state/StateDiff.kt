...
import gg.sona.afterimage.mc263.mixin.EntityEventInvoker
import gg.sona.afterimage.mc263.mixin.RotateHeadInvoker
import gg.sona.afterimage.mc263.mixin.SetEntityLinkInvoker
import gg.sona.afterimage.mc263.mixin.SetPlayerTeamInvoker
...
object StateDiff {
    ...
    fun packets(from: ShadowClient, to: ShadowClient, nanos: Long, freshEntities: Boolean = false): List<CapturedPacket> {
    ...
    private fun diffPlayers(from: ShadowClient, to: ShadowClient, out: MutableList<Packet<*>>) {
        ...
        if (added.isNotEmpty()) out += ShadowPlayerList.addPacket(added.map { it.entry })
        ...
        for (player in changed) {
            ...
            out += ShadowPlayerList.updatePacket(ClientboundPlayerInfoUpdatePacket.Action.UPDATE_GAME_MODE, entries)
            out += ShadowPlayerList.updatePacket(ClientboundPlayerInfoUpdatePacket.Action.UPDATE_LISTED, entries)
            out += ShadowPlayerList.updatePacket(ClientboundPlayerInfoUpdatePacket.Action.UPDATE_LATENCY, entries)
            out += ShadowPlayerList.updatePacket(ClientboundPlayerInfoUpdatePacket.Action.UPDATE_DISPLAY_NAME, entries)
            out += ShadowPlayerList.updatePacket(ClientboundPlayerInfoUpdatePacket.Action.UPDATE_HAT, entries)
            out += ShadowPlayerList.updatePacket(ClientboundPlayerInfoUpdatePacket.Action.UPDATE_LIST_ORDER, entries)
        }
        if (from.players.tabList != to.players.tabList) to.players.tabList?.let { out += it }
    }

    private fun diffLevelMeta(from: ShadowClient, to: ShadowClient, nanos: Long, out: MutableList<Packet<*>>) {
        val source = from.level
        val target = to.level
        for (kind in ShadowWorld.SINGLETON_ORDER) {
            val packet = target.singletons[kind] ?: continue
            if (!packet.same(source.singletons[kind])) out += packet.packet
        }
        SnapshotEncoder.currentTime(to, nanos)?.let { out += it }
    ...
    private fun diffChunks(from: ShadowClient, to: ShadowClient, out: MutableList<Packet<*>>) {
        val source = from.level
        val target = to.level
    ...
    private fun diffEntities(from: ShadowClient, to: ShadowClient, nanos: Long, out: MutableList<Packet<*>>, fresh: Boolean) {
        ...
        val spawned = ArrayList<ShadowEntity>()
        ...
        val ghosts = SnapshotEncoder.ghostProfiles(to, spawned, out)
    ...
    private fun updateEntity(codecs: Codecs, previous: ShadowEntity, current: ShadowEntity, nanos: Long, out: MutableList<Packet<*>>) {
        val id = current.id
        ...
        if (!sameEquipment(previous.equipment, current.equipment)) {
            out += ClientboundSetEquipmentPacket(id, ShadowEntity.slotOrder().map { slot -> Pair.of(slot, current.equipment[slot] ?: net.minecraft.world.item.ItemStack.EMPTY) })
        }
        ...
        if (!codecs.same(previous.link, current.link)) out += current.link ?: EntityLinkPackets.unlink(id)
    ...
    private fun diffScoreboard(from: ShadowClient, to: ShadowClient, out: MutableList<Packet<*>>) {
        val source = from.scoreboard
        val target = to.scoreboard
        for ((name, objective) in source.objectives) {
            if (target.objectives[name] == null) out += ShadowScoreboard.objectivePacket(objective, ClientboundSetObjectivePacket.METHOD_REMOVE)
        }
        for ((name, objective) in target.objectives) {
            val previous = source.objectives[name]
            if (previous == null) {
                out += ShadowScoreboard.objectivePacket(objective, ClientboundSetObjectivePacket.METHOD_ADD)
                out += objective.scorePackets.values
                continue
            }
            if (!sameObjective(previous.packet, objective.packet)) out += ShadowScoreboard.objectivePacket(objective, ClientboundSetObjectivePacket.METHOD_CHANGE)
        ...
        for ((slot, name) in target.display) {
            if (source.display[slot] != name) target.objectives[name]?.let { out += ClientboundSetDisplayObjectivePacket(slot, ShadowScoreboard.dummyObjective(it.packet)) }
        }
        for (name in source.teams.keys) {
            if (target.teams[name] == null) out += SetPlayerTeamInvoker.afterimage_create(name, ShadowTeam.METHOD_REMOVE, Optional.empty(), emptyList())
        }
        for ((name, team) in target.teams) {
            ...
            if (previous.parameters != team.parameters) out += SetPlayerTeamInvoker.afterimage_create(name, ShadowTeam.METHOD_CHANGE, Optional.ofNullable(team.parameters), emptyList())
            val left = previous.members.filter { it !in team.members }
            if (left.isNotEmpty()) out += SetPlayerTeamInvoker.afterimage_create(name, ShadowTeam.METHOD_LEAVE, Optional.empty(), left)
            val joined = team.members.filter { it !in previous.members }
            if (joined.isNotEmpty()) out += SetPlayerTeamInvoker.afterimage_create(name, ShadowTeam.METHOD_JOIN, Optional.empty(), joined)
        }
    }

    private fun diffLocalPlayer(from: ShadowClient, to: ShadowClient, nanos: Long, out: MutableList<Packet<*>>) {
    ...
    private fun diffExtras(from: ShadowClient, to: ShadowClient, out: MutableList<Packet<*>>) {
...
private fun sameData(codecs: Codecs, id: Int, a: Map<Int, SynchedEntityData.DataValue<*>>, b: Map<Int, SynchedEntityData.DataValue<*>>): Boolean {
