...
import gg.sona.afterimage.mc263.replay.ChunkReadiness
...
class McExportPlatform(private val minecraft: Minecraft) : ExportPlatform {
    private inner class Surface(override val width: Int, override val height: Int, private val transparent: Boolean) : ExportSurface {
        ...
        override fun asTarget(): Target = McGfx.mainTarget(minecraft).also { it.color?.setFilter(filter) }

        override fun makeOpaque() {
            McGfx.gfx.clear(asTarget(), 0f, 0f, 0f, 1f, gg.sona.afterimage.gfx.ColorMask.ALPHA)
        }
