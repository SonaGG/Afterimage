...
import gg.sona.afterimage.mc189.state.PacketEvents
import gg.sona.afterimage.mc189.state.SnapshotEncoder
import gg.sona.afterimage.mc189.state.StateDiff

class ShadowClient(identity: RecorderIdentity = RecorderIdentity.UNKNOWN) : ReplayState {
    val level = ShadowWorld()
    ...
    override val identity: RecorderIdentity get() = recorderIdentity

    override val world: WorldState = ShadowView(this)

    var liveEvents: WorldEventSink? = null

    override fun fork(): ReplayState = ShadowClient(recorderIdentity)

    override fun canDiff(target: ReplayState): Boolean = target is ShadowClient && StateDiff.canDiff(this, target)

    override fun diff(target: ReplayState, nanos: Long): List<CapturedPacket> =
        PacketCodec.encodeAll(StateDiff.packets(this, target as ShadowClient, nanos), nanos)

    override fun adoptFrom(target: ReplayState, nanos: Long) {
        val other = target as ShadowClient
    ...
    override fun apply(packet: CapturedPacket, events: WorldEventSink?) {
        ...
        PacketEvents.before(decoded, this, packet.timestampNanos, events)
        apply(decoded, packet.timestampNanos)
        PacketEvents.after(decoded, this, packet.timestampNanos, events)
    }
    ...
    fun apply(packet: PlayPacket, nanos: Long) {
        when (packet) {
            ...
            is TimeUpdate -> {
                level.worldAge = packet.worldAge
                level.timeOfDay = packet.timeOfDay
                level.timeUpdatedAtNanos = nanos
                level.hasTime = true
            }

            is SpawnPosition -> {
                level.spawnPosition = packet.position
                level.hasSpawn = true
            }

            is ServerDifficulty -> level.difficulty = packet.difficulty
            is WorldBorder -> level.border.apply(packet, nanos)
            is ChangeGameState -> gameState(packet)
            is ChunkData -> chunkData(packet, nanos)
            is MapChunkBulk -> for ((chunkX, chunkZ, mask, data) in packet.chunks) level.loadChunk(
                chunkX,
                chunkZ,
                mask,
            ...
            is BlockChange -> level.setBlockState(
            ...
            is MultiBlockChange -> {
                ...
                for ((x, y, z, blockState) in packet.records) level.setBlockState(baseX + x, y, baseZ + z, blockState)
            }

            is UpdateBlockEntity -> {
                if (packet.nbt == null) level.blockEntities.remove(packet.position)
                else level.blockEntities.put(
                    packet.position,
                    BlockEntityRecord(packet.position, packet.action, packet.nbt)
                )
            }

            is UpdateSign -> level.signs.put(packet.position, SignRecord(packet.position, packet.lines))
            ...
            is LocalBlockChange -> level.setBlockState(
                PackedPosition.x(packet.position),
                PackedPosition.y(packet.position),
                PackedPosition.z(packet.position),
                packet.state
    ...
    private fun joinGame(packet: JoinGame) {
        ...
        level.dimension = packet.dimension
        level.difficulty = packet.difficulty
        level.maxPlayers = packet.maxPlayers
        level.levelType = packet.levelType
        joined = true
    }

    private fun respawn(packet: Respawn) {
        if (packet.dimension != level.dimension) {
            level.clear()
            entities.clear()
    ...
    private fun gameState(packet: ChangeGameState) {
        when (packet.reason) {
            ChangeGameState.END_RAINING -> {
                level.raining = false
                level.rainStrength = 0f
            }

            ChangeGameState.BEGIN_RAINING -> level.raining = true
            ChangeGameState.CHANGE_GAME_MODE -> localPlayer.gameMode = packet.value.toInt()
            ChangeGameState.RAIN_STRENGTH -> level.rainStrength = packet.value
            ChangeGameState.THUNDER_STRENGTH -> level.thunderStrength = packet.value
        }
    }

    private fun chunkData(packet: ChunkData, nanos: Long) {
        if (packet.isUnload) {
            level.unloadChunk(packet.chunkX, packet.chunkZ)
            return
        }
        level.loadChunk(
            packet.chunkX,
    ...
    private fun mapData(packet: MapData) {
        val canvas = level.maps.getOrPut(packet.mapId) { MapCanvas(packet.mapId) }
