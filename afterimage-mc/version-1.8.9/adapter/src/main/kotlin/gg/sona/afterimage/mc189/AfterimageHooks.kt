...
object AfterimageHooks {
    ...
    fun bind(runtime: AfterimageRuntime) {
    ...
    fun onReplayDisconnectSuppressed(reason: Text?) {
        logger.debug("Afterimage ignored a disconnect on the replay connection: {}", reason?.string)
    }
    ...
    @JvmStatic
    fun renderTargetOverride(): RenderTarget? = (runtime?.exporter?.activeTarget() as? McExportPlatform.Surface)?.target
    ...
    fun onSoundPlay(engine: SoundEngine, sound: SoundInstance): Boolean {
        val current = runtime ?: return false
        return (current.exporter.sounds as SoundCapture).onPlay(engine, sound)
    }
