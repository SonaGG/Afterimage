...
import gg.sona.afterimage.mc189.ScreenCapture
import gg.sona.afterimage.mc189.ItemStacks
...
import gg.sona.afterimage.mc189.state.ShadowWindow
...
class ScreenMirror(
    ...
    fun close() {
        ...
        if (current is CreativeInventoryScreen) {
            val listener = (current as CreativeInventoryScreenAccessor).`afterimage$listener`()
        ...
        when (target) {
            ...
            is InventoryMenuScreen -> {
                ...
                if (target is AnvilScreen && textChanged(state)) {
                    val field = (target as AnvilScreenAccessor).`afterimage$renameField`()
                    if (field != null) {
                ...
                if (target is VillagerScreen && detailChanged(state)) {
                    (target as VillagerScreenAccessor).`afterimage$setCurrentPage`(state.detail)
    ...
    private fun syncCreative(target: CreativeInventoryScreen, state: LocalScreen) {
        ...
        withCreativeMode(target) {
            if (tabChanged) {
                ...
                accessor.`afterimage$selectTab`(tab)
            }
            val field = accessor.`afterimage$searchField`()
            if (field != null && (text || tabChanged) && state.detail == CreativeModeTab.SEARCH.getId()) {
                field.text = state.text
                field.cursor = state.text.length
                accessor.`afterimage$search`()
            }
            accessor.`afterimage$setScrollPosition`((state.cursor / ScreenCapture.SCROLL_SCALE).coerceIn(0f, 1f))
        }
    }

    private fun syncBook(target: BookEditScreen, state: LocalScreen) {
        val accessor = target as BookEditScreenAccessor
        val pages = accessor.`afterimage$pages`() ?: NbtList()
        if (!accessor.`afterimage$unsigned`()) {
            accessor.`afterimage$setCurrentPage`(state.detail.coerceIn(0, maxOf(0, pages.size() - 1)))
            return
        }
        val signing = state.cursor == 1
        accessor.`afterimage$setSigning`(signing)
        if (signing) {
            accessor.`afterimage$setTitle`(state.text)
            return
        }
        while (pages.size() <= state.detail) pages.addElement(NbtString(""))
        pages.setElement(state.detail, NbtString(state.text))
        accessor.`afterimage$setPageCount`(maxOf(1, pages.size()))
        accessor.`afterimage$setCurrentPage`(state.detail.coerceIn(0, pages.size() - 1))
    }
    ...
    private inline fun withCreativeMode(target: Screen, block: () -> Unit) {
        ...
        if (manager != null && previous != WorldSettings.GameMode.CREATIVE) (manager as ClientPlayerInteractionManagerAccessor).`afterimage$setRawGameMode`(
            WorldSettings.GameMode.CREATIVE
        )
        try {
            block()
        } finally {
            if (manager != null && previous != null && previous != WorldSettings.GameMode.CREATIVE) (manager as ClientPlayerInteractionManagerAccessor).`afterimage$setRawGameMode`(
                previous
            )
        }
    }
    ...
    private class MirrorSignScreen(private val target: SignBlockEntity) : SignEditScreen(target) {
        ...
        fun setLines(text: String, row: Int) {
            ...
            (this as SignEditScreenAccessor).`afterimage$setRow`(target.currentRow)
        }
