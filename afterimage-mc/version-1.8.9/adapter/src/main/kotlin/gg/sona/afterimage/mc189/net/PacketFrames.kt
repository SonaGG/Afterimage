package gg.sona.afterimage.mc189.net
...
import gg.sona.afterimage.protocol.AfterimageChannel
...
object PacketFrames {
    ...
    private fun supplementary(sink: PacketSink, bytes: ByteArray, offset: Int) {
        try {
            ...
            if (channel != AfterimageChannel.NAME) return
            for (packet in AfterimageChannel.translate(reader.readRemaining())) {
                val encoded = PacketCodec.encode(packet, 0L)
                sink.offer(PacketDirection.CLIENTBOUND, encoded.packetId, encoded.payload)
            }
        } catch (error: Throwable) {
            if (warned.add(AfterimageChannelMarker::class.java)) logger.warn(
                "Afterimage could not translate an afterimage:v1 message",
                error
            )
        }
    }

    private object AfterimageChannelMarker
    ...
    fun capture(sink: PacketSink, direction: PacketDirection, packet: Packet<*>) {
        ...
        try {
            ...
            if (warned.add(packet.javaClass)) logger.warn(
                "Afterimage could not encode local packet {}",
